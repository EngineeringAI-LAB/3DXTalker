...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256, rasterizer_type='pytorch3d'):
        ...
        if rasterizer_type == 'pytorch3d':
            ...
            my_dense_faces = np.load('/scratch3/wan451/3DTalk/DECA2/data/my_dense_faces_6.npy')
            my_dense_faces = torch.from_numpy(my_dense_faces-1).long()[None,:,:]
        ...
        self.register_buffer('my_dense_faces', my_dense_faces)
        ...
        dense_colors = torch.tensor([180, 180, 180])[None, None, :].repeat(1, my_dense_faces.max()+1, 1).float()/255.
        my_dense_face_colors = util.face_vertices(dense_colors, my_dense_faces)
        self.register_buffer('my_dense_face_colors', my_dense_face_colors)
    ...
    def render_shape(self, vertices, transformed_vertices, colors = None, images=None, detail_normal_images=None, 
                lights=None, return_grid=False, uv_detail_normals=None, h=None, w=None, dense_type=False):
        '''
        ...
        if colors is None:
            if dense_type:
                colors = self.my_dense_face_colors.expand(batch_size, -1, -1, -1)
            else:
                colors = self.face_colors.expand(batch_size, -1, -1, -1)
        if dense_type:
            attributes = torch.cat([colors,     # [1, 9976, 3, 3]
                            transformed_face_normals.detach(),   # [1, 9976, 3, 3]
                            face_vertices.detach(),    # [1, 9976, 3, 3]
                            face_normals,              # [1, 9976, 3, 3]
                            # self.face_uvcoords.expand(batch_size, -1, -1, -1)],  # [1, 9976, 3, 3]
                            ],
                           -1)
        else:
            attributes = torch.cat([colors,     # [1, 9976, 3, 3]
