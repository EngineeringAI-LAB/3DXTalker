...
def upsample_mesh_batch(vertices, normals, displacement_map, dense_template):
    """
    Batched mesh upsampling using displacement map and normal map.
    Args:
        vertices: (B, V, 3)
        normals: (B, V, 3)
        faces: (F, 3) - not used in computation, just output
        displacement_map: (B, 1, 256, 256)
        texture_map: (B, 256, 256, 3)
        dense_template: dict with keys:
            - x_coords: (Npx,)
            - y_coords: (Npx,)
            - valid_pixel_ids: (Np,)
            - valid_pixel_3d_faces: (Np, 3)
            - valid_pixel_b_coords: (Np, 3)
            - f: dense_faces: (Nf, 3)
    Returns:
        dense_vertices: (B, Np, 3)
        dense_colors: (B, Np, 3)
        dense_faces: (Nf, 3)
    """
    ...
    x_coords = torch.tensor(dense_template['x_coords'], dtype=torch.long, device=device)
    y_coords = torch.tensor(dense_template['y_coords'], dtype=torch.long, device=device)
    valid_pixel_ids = torch.tensor(dense_template['valid_pixel_ids'], dtype=torch.long, device=device)
    valid_pixel_3d_faces = torch.tensor(dense_template['valid_pixel_3d_faces'].astype(int), dtype=torch.long, device=device)
    valid_pixel_b_coords = torch.tensor(dense_template['valid_pixel_b_coords'], dtype=torch.float32, device=device)

    # Np = valid_pixel_ids.shape[0]

    x_pix = x_coords[valid_pixel_ids]  # (Np,)
    y_pix = y_coords[valid_pixel_ids]

    # x_pix_expanded = x_pix.unsqueeze(0).expand(batch_size, -1)
    # y_pix_expanded = y_pix.unsqueeze(0).expand(batch_size, -1)


    # Interpolate vertex positions
    v0, v1, v2 = valid_pixel_3d_faces[:, 0], valid_pixel_3d_faces[:, 1], valid_pixel_3d_faces[:, 2]
    # b0, b1, b2 = valid_pixel_b_coords[:, 0][None, :, None], valid_pixel_b_coords[:, 1][None, :, None], valid_pixel_b_coords[:, 2][None, :, None]
    
    b0 = valid_pixel_b_coords[:, 0].unsqueeze(-1)  # (Np, 1)
    b1 = valid_pixel_b_coords[:, 1].unsqueeze(-1)
    b2 = valid_pixel_b_coords[:, 2].unsqueeze(-1)

    p0 = vertices[:, v0, :]  # (B, Np, 3)
    p1 = vertices[:, v1, :]
    p2 = vertices[:, v2, :]
    points = p0 * b0 + p1 * b1 + p2 * b2  # (B, Np, 3)

    # Interpolate normals
    n0 = normals[:, v0, :]
    n1 = normals[:, v1, :]
    n2 = normals[:, v2, :]
    normals_interp = n0 * b0 + n1 * b1 + n2 * b2
    normals_interp = torch.nn.functional.normalize(normals_interp, dim=-1)


    # Sample displacement
    displacements = displacement_map[:, 0, y_pix, x_pix]  # (B, Np)

    # Apply offset
    offsets = displacements.unsqueeze(-1) * normals_interp  # (B, Np, 3)
    dense_vertices = points + offsets  # (B, Np, 3)

    # Sample texture
    # dense_colors = texture_map[:, y_pix, x_pix, :]  # (B, Np, 3)
...
def upsample_mesh(vertices, normals, faces, displacement_map, texture_map, dense_template):
    ...
    use_my_dense_face = True
    if use_my_dense_face:
        dense_faces = np.load('/scratch3/wan451/3DTalk/DECA2/data/my_dense_faces_6.npy')   # [117437, 3]   diff 57 faces
        if dense_faces.min() == 1:
            dense_faces = dense_faces-1
    else:
        dense_faces = dense_template['f']   # [117380, 3]

    x_coords = dense_template['x_coords']                            # (65536, )    (256, 256, 2)     (5118, 2) 
    y_coords = dense_template['y_coords']                            # (65536, )
    valid_pixel_ids = dense_template['valid_pixel_ids']              # (59315, )
    valid_pixel_3d_faces = dense_template['valid_pixel_3d_faces']    # (59315, )
    valid_pixel_b_coords = dense_template['valid_pixel_b_coords']    # (59315, )
...
def deatil_mesh(vertices, normals, uvs, displacement_map):
    H, W = displacement_map.shape[1:]
    u = uvs[:, 0] * (W-1)
    v = (1- uvs[:, 1])* (H-1)
    disp = displacement_map[0, v.astype(int), u.astype(int)]
