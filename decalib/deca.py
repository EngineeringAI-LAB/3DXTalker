...
class DECA(nn.Module):
    ...
    def displacement2normal(self, uv_z, coarse_verts, coarse_normals):
        ...
        uv_detail_normals = uv_detail_normals*self.uv_face_mask + uv_coarse_normals*(1.-self.uv_face_mask)
    ...
    def encode(self, images, use_detail=True):
        ...
        if self.cfg.model.jaw_type == 'euler':
            ...
            # posecode[:,:3] = 0
            posecode[:, 3] = posecode[:, 3] * 1.5
            # posecode[:, 4] = posecode[:, 4] * 1.5
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        # temp_shape = torch.zeros_like(codedict['shape'])
        # temp_exp = torch.zeros_like(codedict['exp'])
        # temp_pose = torch.zeros_like(codedict['pose'])

        ## decode
        verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=codedict['pose'])
        # verts, landmarks2d, landmarks3d = self.flame(shape_params=temp_shape, expression_params=temp_exp, pose_params=temp_pose)

        if self.cfg.model.use_tex:
            albedo = self.flametex(codedict['tex'])      # (1, 3, 256, 256)
        else:
            albedo = torch.zeros([batch_size, 3, self.uv_size, self.uv_size], device=images.device) 
        landmarks3d_world = landmarks3d.clone()

        # codedict['cam'] = torch.zeros_like(codedict['cam']).to(codedict['cam'].device)
        ...
        if use_detail:
            ...
            # uv_texture = albedo*uv_shading                                               # [1, 3, 256, 256]
            uv_texture = albedo
            ...
            texture = texture[:,:,[2,1,0]]                                              # (256, 256, 3)

            def img_output(x_tensor):
                # x_tensor : [256, 256]
                x_min = x_tensor.min()
                x_max = x_tensor.max()
                x_norm = (x_tensor - x_min) / (x_max - x_min + 1e-8)
                x_uint8 = (x_norm * 255).byte()
                x_img = torchvision.transforms.ToPILImage()(x_uint8)
                return x_img
                
            save_displacemet_map = False
            if save_displacemet_map:
                ori_disp = uv_z.squeeze().detach().cpu() 
                ori_disp_img = img_output(ori_disp)
                ori_disp_img.save("ori_displacement_map.png")

                fixed_disp= self.fixed_uv_dis.detach().cpu() 
                fixed_disp_img = img_output(fixed_disp)
                fixed_disp_img.save("fixed_displacement_map.png")

                hybird_disp = opdict['displacement_map'].squeeze().detach().cpu()
                hybird_disp_img = img_output(hybird_disp)
                hybird_disp_img.save("hybird_displacement_map.png")
            ...
            dense_vertices, dense_colors, dense_faces = util.upsample_mesh(
                                                                            verts[0].cpu().numpy(),
                                                                            opdict['normals'][0].cpu().numpy(),
                                                                            self.render.faces[0].cpu().numpy(),
                                                                            opdict['displacement_map'][0].cpu().numpy().squeeze(),
                                                                            texture,
                                                                            self.dense_template,
                                                                        )
            
            dense_vertices = torch.from_numpy(dense_vertices)[None, ...].to(self.device)
            trans_dense_verts = util.batch_orth_proj(dense_vertices, codedict['cam']); trans_dense_verts[:,:,1:] = -trans_dense_verts[:,:,1:]

            opdict['dense_verts'] = dense_vertices
            opdict['trans_dense_verts'] = trans_dense_verts
        ...
        if return_vis:
            ...
            shape_detail_images, _, grid, alpha_images = self.render.render_shape(dense_vertices, trans_dense_verts, h=h, w=w, images=background, return_grid=True, dense_type=True)
            ...
            if self.cfg.model.use_tex:
                ...
                # uv_texture_gt = uv_gt[:,:3,:,:]*self.uv_face_mask + (torch.ones_like(uv_gt[:,:3,:,:])*(1-self.uv_face_mask)*0.7)
    ...
    def visualize(self, visdict, size=224, dim=2):
        ...
        grids = {}
        for key in visdict:
            if key in ['inputs', 'shape_images', 'shape_detail_images']:
    ...
    def save_obj(self, filename, opdict):
        ...
        uvcoords = self.render.raw_uvcoords[0].cpu().numpy()   # (5118, 2)
        uvfaces = self.render.uvfaces[0].cpu().numpy()         # (9976, 2)
